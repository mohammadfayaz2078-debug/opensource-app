  ...
  useEffect(() => {
    const fetchReceipt = async () => {
      try {
        ...
        setError(err.response?.data?.message || t('purchase.load_receipt_failed'));
      } finally {
        setLoading(false);
      }
    };
  ...
  if (error || !data) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50">
        <p className="text-red-600 mb-4">{error || t('purchase.receipt_not_found')}</p>
        <button onClick={() => navigate(-1)} className="text-sm text-[#007c89] hover:underline">&larr; {t('purchase.go_back')}</button>
      </div>
    );
  }
      ...
      <div ref={receiptRef} className="max-w-2xl mx-auto px-4">
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden print:shadow-none print:border-none print:rounded-none">
          ...
          <div className="bg-gradient-to-r from-[#007c89] to-[#006d77] px-6 py-5 text-white print:bg-gray-800">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-lg font-bold tracking-tight">{t('purchase.payment_receipt')}</h1>
                <p className="text-sm text-white/80 mt-0.5">{t('purchase.receipt_no', { id: transaction.id })}</p>
              </div>
              <div className="text-right">
                <p className="text-sm font-medium">{receiptDate}</p>
                <p className="text-xs text-white/70">{t('purchase.date_of_payment')}</p>
              </div>
            </div>
          </div>
          ...
          <div className="p-6 space-y-5">
            ...
            <div className="border-b border-gray-100 pb-4">
              <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">{t('purchase.payment_details')}</h2>
              <div>
                <p className="text-xs text-gray-400">{t('purchase.amount_paid')}</p>
            ...
            {/* Bill Info */}
            <div className="border-b border-gray-100 pb-4">
              <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">{t('purchase.bill_information')}</h2>
              <div className="grid grid-cols-2 gap-y-2.5 text-sm">
                <div>
                  <span className="text-gray-400">{t('purchase.bill_no')}</span>
                  <p className="font-medium text-gray-900">#{purchase.id}</p>
                </div>
                <div className="text-right">
                  <span className="text-gray-400">{t('purchase.bill_total')}</span>
                  <p className="font-medium text-gray-900">{parseFloat(purchase.total_amount).toFixed(2)} AFN</p>
                </div>
                <div>
                  <span className="text-gray-400">{t('purchase.supplier')}</span>
                  <p className="font-medium text-gray-900">{purchase.supplier?.full_name || '—'}</p>
                </div>
                <div className="text-right">
                  <span className="text-gray-400">{t('purchase.remaining_due')}</span>
                  <p className="font-medium text-red-600">{parseFloat(purchase.due_amount).toFixed(2)} AFN</p>
                </div>
                <div>
                  <span className="text-gray-400">{t('purchase.payment_status')}</span>
            ...
            {/* Account Info */}
            <div className="border-b border-gray-100 pb-4">
              <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">{t('purchase.payment_source')}</h2>
            ...
            {transaction.description && (
              <div className="pb-2">
                <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">{t('purchase.description')}</h2>
            ...
            <div className="pt-2 text-center">
              <div className="border-t border-gray-100 pt-4">
                <p className="text-xs text-gray-400">{t('purchase.receipt_footer')}</p>
                <p className="text-xs text-gray-400 mt-0.5">{t('purchase.transaction_id', { id: transaction.id })}</p>
              </div>
            </div>
          </div>
        </div>
