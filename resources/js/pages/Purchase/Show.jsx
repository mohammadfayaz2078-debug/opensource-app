...
export default function PurchaseShow() {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      alert(err.response?.data?.message || t('purchase.payment_failed'));
    } finally { setPaying(false); }
  };

  const handleReturn = () => {
    navigate(`/purchase-returns/create?purchase_id=${id}`);
  };
  ...
  const canPay = () => {
    return canTakeAction() && purchase?.payment_status !== 'paid';
  };

  const canReturn = () => {
    return canTakeAction() && purchase?.payment_status !== 'unpaid' && purchase?.refund_status !== 'full';
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-sm text-gray-500">{t('purchase.loading_details')}</span>
        </div>
      </div>
    );
  }
  ...
  const refundBadge = (status) => {
    ...
    return (
      <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium ${refundColors[status] || 'bg-gray-100 text-gray-500'}`}>
        {status === 'full' && (
  ...
  const refundStatusBadge = (item) => {
    const status = item.refund_status || 'none';
    const colors = {
      none: 'bg-gray-100 text-gray-500',
      partial: 'bg-yellow-100 text-yellow-700',
      full: 'bg-purple-100 text-purple-700'
    ...
    const labels = { none: t('purchase.refund_none'), partial: t('purchase.refund_partial'), full: t('purchase.refund_full') };
    return (
      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium ${colors[status]}`}>
        {status === 'full' && (
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mb-6">
        ...
        >
          ...
          {t('purchase.back_to_purchases')}
        </button>
        
        <div className="flex flex-wrap items-start justify-between gap-4 mt-3">
          <div className="flex items-center flex-wrap gap-3">
            <h1 className="text-2xl font-bold text-gray-900">{purchase.reference_no || `Bill #${purchase.id}`}</h1>
            <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${paymentColors[purchase.payment_status] || 'bg-gray-100 text-gray-700'}`}>
              {({ unpaid: t('payment.unpaid'), partial: t('payment.partial'), paid: t('payment.paid') })[purchase.payment_status] || (purchase.payment_status || '').toUpperCase()}
            </span>
            {refundBadge(purchase.refund_status)}
          ...
          <div className="flex flex-wrap gap-2">
            <button 
              onClick={() => navigate(`/purchases/${id}/invoice`)}
              className="inline-flex items-center gap-2 px-4 py-2 text-sm bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors shadow-sm"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {canReturn() && (
              ...
              >
                ...
                {t('purchase.process_return')}
              </button>
            )}

            {canPay() && (
              <button 
                onClick={() => { setPayAccountId(purchase?.account_id || ''); setShowPayModal(true); }}
            ...
            {canTakeAction() && purchase.payment_status === 'unpaid' && purchase.refund_status === 'none' && (
              <button 
                onClick={() => navigate(`/purchases/${id}/edit`)}
                className="inline-flex items-center gap-2 px-4 py-2 bg-amber-50 text-amber-700 text-sm font-medium rounded-lg hover:bg-amber-100 transition-all border border-amber-200"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200 bg-gray-50/50 flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                <span className="text-xs text-gray-500">{t('purchase.items_count', { count: purchase.items?.length || 0 })}</span>
                {purchase.refund_status === 'full' && (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-200 bg-gray-50/30 text-xs text-gray-500 uppercase">
                    <th className="text-left py-3 px-4 font-medium w-12">{t('purchase.col_hash')}</th>
                    <th className="text-left py-3 px-4 font-medium">{t('purchase.col_product')}</th>
                    <th className="text-right py-3 px-4 font-medium w-20">{t('purchase.col_qty')}</th>
                    <th className="text-right py-3 px-4 font-medium w-24">{t('purchase.col_price')}</th>
                    <th className="text-right py-3 px-4 font-medium w-24">{t('purchase.col_total')}</th>
                    <th className="text-center py-3 px-4 font-medium w-28">{t('purchase.col_refund_status')}</th>
                    <th className="text-right py-3 px-4 font-medium w-20">{t('purchase.col_refund_qty')}</th>
                    <th className="text-right py-3 px-4 font-medium w-24">{t('purchase.col_refund_amount')}</th>
                  </tr>
                </thead>
                <tbody>
                          ...
                          {hasRefund && (
                            <div className="mt-1">
                              ...
                              <div className="text-[10px] text-gray-400 mt-0.5">
                                {t('purchase.refunded_pct', { pct: progressPercentage.toFixed(0) })}
                              </div>
                            </div>
                          )}
                        </td>
                ...
                {purchase.items?.some(item => item.refund_status && item.refund_status !== 'none') && (
                  <tfoot>
                    <tr className="border-t-2 border-gray-200 bg-gray-50/50">
                      <td colSpan="4" className="py-3 px-4 text-right font-semibold text-gray-900">
                        {t('purchase.total_refunded')}
                      </td>
                      <td className="py-3 px-4 text-right font-semibold text-gray-900">
                        {purchase.items?.reduce((sum, item) => sum + parseFloat(item.refunded_amount || 0), 0).toFixed(2)}
                      </td>
                      <td colSpan="3" className="py-3 px-4 text-right text-xs text-gray-500">
                        {t('purchase.fully_refunded_items', { count: purchase.items?.filter(item => item.refund_status === 'full').length })}
                      </td>
                    </tr>
                  </tfoot>
                )}
          ...
          {purchase.returns?.length > 0 && (
            <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-200 bg-gray-50/50 flex items-center justify-between">
                ...
                <span className="text-xs text-gray-500">{t('purchase.returns_count', { count: purchase.returns.length })}</span>
              </div>
              <div className="p-4 space-y-2">
                {purchase.returns.map(ret => (
                  <div key={ret.id} className="flex justify-between items-center p-3 bg-red-50/50 rounded-lg border border-red-100 hover:bg-red-50 transition-colors">
                    <div>
                      <span className="font-medium text-sm text-gray-900">{ret.reference_no}</span>
                      <span className="text-xs text-gray-500 ml-3">{ret.return_date?.split('T')[0]}</span>
                      {ret.reason && (
                        <span className="text-xs text-gray-400 ml-3">{t('purchase.reason_colon', { reason: ret.reason })}</span>
                      )}
                    </div>
          ...
          {payments.length > 0 && (
            <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-200 bg-gray-50/50 flex items-center justify-between">
                ...
                <span className="text-xs text-gray-500">{t('purchase.transactions_count', { count: payments.length })}</span>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-200 bg-gray-50/30 text-xs text-gray-500 uppercase">
                      <th className="text-left py-3 px-6 font-medium">{t('purchase.date')}</th>
                      <th className="text-right py-3 px-6 font-medium">{t('purchase.amount')}</th>
                      <th className="text-right py-3 px-6 font-medium">{t('purchase.balance')}</th>
                      <th className="text-center py-3 px-6 font-medium">{t('purchase.receipt')}</th>
                    </tr>
                  </thead>
                  <tbody>
          ...
          <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
            ...
            <div className="p-6 space-y-3">
              <div className="flex justify-between text-sm">
                ...
                <span className="text-gray-900">{parseFloat(purchase.subtotal).toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">{t('purchase.discount')}</span>
                <span className="text-red-600">-{parseFloat(purchase.discount_value).toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">{t('purchase.shipping')}</span>
                <span className="text-gray-900">{parseFloat(purchase.shipping_cost).toFixed(2)}</span>
              </div>
              <div className="border-t border-gray-200 pt-3">
                <div className="flex justify-between text-base font-bold">
                  <span className="text-gray-900">{t('purchase.total')}</span>
                  <span className="text-gray-900">{parseFloat(purchase.total_amount).toFixed(2)}</span>
                </div>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-green-600 font-medium">{t('purchase.paid')}</span>
                <span className="text-green-600 font-medium">{parseFloat(purchase.paid_amount).toFixed(2)}</span>
              </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
            ...
            <div className="p-6 space-y-3 text-sm">
              <div className="flex justify-between">
                <span className="text-gray-500">{t('purchase.supplier')}</span>
                <span className="text-gray-900 font-medium">{purchase.supplier?.full_name || '—'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">{t('purchase.date')}</span>
                <span className="text-gray-900">{purchase.purchase_date?.split('T')[0]}</span>
              </div>
              {purchase.due_date && (
                <div className="flex justify-between">
                  <span className="text-gray-500">{t('purchase.due_date')}</span>
                  <span className="text-gray-900">{purchase.due_date?.split('T')[0]}</span>
                </div>
              )}
              <div className="flex justify-between">
                <span className="text-gray-500">{t('purchase.wallet')}</span>
                <span className="text-gray-900">{purchase.account?.name || '—'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">{t('purchase.created_by')}</span>
                <span className="text-gray-900">{purchase.creator?.first_name + ' ' + purchase.creator?.last_name || '—'}</span>
              </div>
              {purchase.notes && (
                <div className="pt-3 border-t border-gray-200">
                  <span className="text-gray-500 block mb-1">{t('purchase.notes')}</span>
            ...
            <div className="px-6 py-6 space-y-4">
              <div className="bg-gray-50 rounded-xl p-5">
                <div className="grid grid-cols-3 gap-4 text-center">
                  <div>
                    <p className="text-xs text-gray-500 uppercase font-medium">{t('purchase.total')}</p>
                    <p className="text-xl font-bold text-gray-900">{parseFloat(purchase.total_amount).toFixed(2)}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 uppercase font-medium">{t('purchase.paid')}</p>
                    <p className="text-xl font-bold text-green-600">{parseFloat(purchase.paid_amount).toFixed(2)}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 uppercase font-medium">{t('purchase.due')}</p>
              ...
              <div>
                ...
                <input 
                  ...
                  autoFocus
                />
                <p className="text-xs text-gray-400 mt-1.5">{t('purchase.maximum', { max: unpaid.toFixed(2) })}</p>
              </div>
