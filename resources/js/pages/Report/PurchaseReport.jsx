...
const PurchaseReport = () => {
  ...
  const fetchFilterOptions = async () => {
    try {
      const res = await api.get('/purchase-report/filters');
  ...
  const handleExport = async () => {
    try {
      ...
      const res = await api.get('/purchase-report/export', { params });
      const blob = new Blob([JSON.stringify(res.data, null, 2)], { type: 'application/json' });
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `purchase-report-${new Date().toISOString().split('T')[0]}.json`;
      a.click();
  ...
  const getGrowthPercentage = () => {
    if (!data?.chart_data?.total || data.chart_data.total.length < 2) return 0;
    const totals = data.chart_data.total;
  ...
  const chartData = data?.chart_data || { labels: [], total: [], count: [], paid: [], due: [] };
  ...
  const spendingChartData = chartData.labels.map((label, index) => ({
    name: label,
  ...
  const refundData = data?.refund_breakdown?.map(item => ({
    name: item.status.toUpperCase(),
    value: item.total,
    count: item.count,
  })) || [];
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2">
              <div>
                ...
                <input
                  ...
                  className="w-full px-2 py-1.5 text-xs border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500"
                />
              </div>
              <div>
              ...
              <div>
                ...
                >
                  <option value="">All</option>
                  {filterOptions?.refund_statuses?.map(s => (
                    <option key={s.value} value={s.value}>{s.label}</option>
                  ))}
                </select>
              </div>
            ...
            <div className="h-64">
              {chartTab === 'spending' && (
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={spendingChartData}>
                    ...
                    />
                    <Legend />
                    <Area type="monotone" dataKey="total" stroke="#8B5CF6" fill="#C4B5FD" fillOpacity={0.3} />
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-3">
            ...
            <div className="space-y-2">
              {topSuppliers.slice(0, 5).map((supplier, index) => (
                <div key={supplier.supplier_id} className="flex items-center justify-between p-2 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                  <div className="flex items-center gap-2 min-w-0">
                    <span className="text-xs font-medium text-gray-400 w-5">#{index + 1}</span>
                    <div className="min-w-0 flex-1">
                  ...
                  purchases.map((purchase) => (
                    <tr key={purchase.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-3 py-2 text-center">
                        <span className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-medium ${getPaymentColor(purchase.payment_status)}`}>
                          {purchase.payment_status?.charAt(0).toUpperCase()}
                        </span>
                      </td>
                      <td className="px-3 py-2 text-center">
                        <span className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-medium ${getRefundColor(purchase.refund_status)}`}>
                          {purchase.refund_status?.charAt(0).toUpperCase()}
                        </span>
                      </td>
                    </tr>
                  ))
