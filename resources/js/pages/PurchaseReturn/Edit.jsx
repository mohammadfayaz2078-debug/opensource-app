...
export default function PurchaseReturnEdit() {
  ...
  const fetchReturnData = async () => {
    ...
    try {
      ...
      const mappedItems = (data.items || []).map(item => ({
        ...
        name: item.product?.name || t('purchase_return.unknown_product'),
      ...
      setErrors({ general: t('purchase_return.load_failed') });
    } finally {
      setFetching(false);
    }
  };
  ...
  const removeItem = (index) => {
    if (items.length <= 1) {
      setErrors({ general: t('purchase_return.cannot_remove_last') });
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (items.length === 0) { 
      setErrors({ general: t('purchase_return.please_add_item') }); 
      return; 
    }
    ...
    if (invalidItems.length > 0) {
      setErrors({ general: t('purchase_return.quantity_positive') });
      return;
    }
    
    if (!hasChanges()) {
      setErrors({ general: t('purchase_return.no_changes') });
      return;
    }
    ...
    try {
      ...
      if (err.response?.status === 422) {
        ...
        setErrors({ general: err.response?.data?.message || t('purchase_return.update_failed') });
      }
    } finally { 
      setLoading(false); 
    }
    ...
    return (
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-center py-20">
          ...
          <span className="ml-3 text-gray-700 text-sm">{t('purchase_return.loading_details')}</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-4">
        ...
        <h1 className="text-xl font-semibold text-gray-900 mt-1">{t('purchase_return.edit_title')}</h1>
      </div>
      ...
      <form onSubmit={handleSubmit}>
          ...
          <div className="p-4">
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              <div>
                ...
                <input
                  type="text"
                  value={t('purchase_return.po_ref', { id: form.purchase_id })}
                  disabled
                  className="w-full px-2 py-1.5 text-sm border border-gray-200 rounded bg-gray-50 text-gray-600"
                />
                <p className="text-xs text-gray-400 mt-1">{t('purchase_return.po_cannot_change')}</p>
              </div>
              <div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm mb-4">
          <div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
            ...
            <span className="text-xs text-gray-500">{t('purchase_return.items_count', { count: items.length })}</span>
          </div>

          {/* Product Search */}
          <div className="p-4 border-b border-gray-200 bg-gray-50/50">
            <div className="max-w-sm">
              <label className="block text-xs text-gray-500 mb-0.5">{t('purchase_return.add_product')}</label>
          ...
          <div className="p-4">
            {items.length === 0 ? (
              <p className="text-sm text-gray-400 text-center py-4">{t('purchase_return.no_items_added')}</p>
            ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-200 text-xs text-gray-500 uppercase">
                      ...
                      <th className="text-left py-2 px-3 font-medium w-48">{t('purchase_return.col_notes')}</th>
                      ...
                      return (
                        <tr key={idx} className="border-b border-gray-100 hover:bg-gray-50">
                          ...
                          <td className="py-2 px-3">
                            ...
                            {item.id && (
                              <div className="text-xs text-gray-400">{t('purchase_return.id_label', { id: item.id })}</div>
                            )}
                          </td>
                          <td className="py-2 px-3">
                          ...
                          <td className="py-2 px-3">
                            <input type="text" value={item.notes || ''}
                              ...
                              placeholder={t('purchase_return.optional')}
          ...
          <div className="lg:col-span-2">
            <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-4">
              ...
              <textarea name="notes" value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })}
                rows="2" className={inputClass} placeholder={t('purchase_return.notes_additional')} />
            </div>
          </div>
          <div>
            <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-4">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between font-bold text-lg border-t pt-2">
                  <span>{t('purchase_return.total')}</span>
              ...
              <button type="submit" disabled={loading || !hasChanges()}
                ...
                {loading ? t('purchase_return.updating') : t('purchase_return.update_return')}
              </button>
