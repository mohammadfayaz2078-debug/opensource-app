...
export default function WarehouseTowerEdit() {
  const { id } = useParams();
  ...
  const handleSubmit = async (e) => {
    ...
    setErrors({});
    try {
      await api.put(`/warehouse-towers/${id}`, form);
      navigate(`/warehouse-towers/${id}`);
    } catch (err) {
      if (err.response?.status === 422) {
        ...
        setErrors({ general: err.response?.data?.message || 'Failed to update location.' });
      }
    } finally {
      setLoading(false);
    }
    ...
    <div>
      ...
      <div className="mb-6">
        <div className="flex items-center gap-2 text-sm text-gray-500 mb-2">
          ...
          <button onClick={() => navigate(`/warehouse-towers/${id}`)} className="hover:text-[#007c89]">
            {form.name}
          </button>
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-6 sticky top-6">
              ...
              <button
                type="button"
                onClick={() => navigate(`/warehouse-towers/${id}`)}
