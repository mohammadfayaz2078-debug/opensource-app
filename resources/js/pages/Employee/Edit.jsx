...
export default function EmployeeEdit() {
  ...
  useEffect(() => {
    ...
    }).catch(() => navigate('/employees')).finally(() => setFetching(false));
  }, [id]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response?.status === 422) {
        ...
        setErrors({ general: err.response?.data?.message || 'Failed to update employee.' });
      }
    } finally {
      setLoading(false);
    }
  ...
  if (fetching) {
    return (
      <div className="flex items-center justify-center py-20">
        ...
        <span className="ml-3 text-gray-600">Loading employee...</span>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="mb-6">
        <div className="flex items-center gap-2 text-sm text-gray-500 mb-2">
          ...
          <button onClick={() => navigate(`/employees/${id}`)} className="hover:text-[#007c89]">{form.first_name} {form.last_name}</button>
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7" />
          </svg>
              ...
              <div className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input name="employee_code" value={form.employee_code} onChange={handleChange} className={inputClass('employee_code')} />
            ...
            <div className="bg-white border border-gray-200 rounded-lg shadow-sm">
              ...
              <div className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div><label className="block text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Village</label><input name="village" value={form.village} onChange={handleChange} className={inputClass('village')} /></div>
                  <div><label className="block text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">District</label><input name="district" value={form.district} onChange={handleChange} className={inputClass('district')} /></div>
                  <div><label className="block text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Province</label><input name="province" value={form.province} onChange={handleChange} className={inputClass('province')} /></div>
                  <div><label className="block text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Country</label><input name="country" value={form.country} onChange={handleChange} className={inputClass('country')} /></div>
                </div>
              </div>
            </div>
              ...
              <div className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <input name="qualifications" value={form.qualifications} onChange={handleChange} className={inputClass('qualifications')} />
                  </div>
                </div>
              </div>
            </div>
