                ...
                <tbody>
                  {filteredProducts.length === 0 ? (
                    ...
                    <tr key={p.id} className={`border-t border-gray-100 ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'} hover:bg-blue-50/50`}>
                      ...
                      <td className="px-4 py-2.5 text-sm text-gray-600 whitespace-nowrap">{p.category?.name || '—'}</td>
                      <td className="px-4 py-2.5 text-sm text-gray-900 text-right whitespace-nowrap">{parseFloat(p.sale_price || 0).toFixed(2)}</td>
                ...
                <div key={p.id} className="p-4 hover:bg-gray-50/50 transition-colors">
                  <div className="flex items-start justify-between mb-2">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-xs text-gray-500 mt-0.5">{p.category?.name || '—'}</div>
                    </div>
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div className="text-sm font-bold text-gray-900">{parseFloat(p.sale_price || 0).toFixed(2)} AFN</div>
                  ...
                  <tbody>
                    {orders.map((o, idx) => (
                      <tr key={o.id} className={`border-t border-gray-100 ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'} hover:bg-blue-50/50`}>
                        <td className="px-4 py-2.5 font-medium text-gray-900 whitespace-nowrap">{o.order_no}</td>
                        <td className="px-4 py-2.5 text-sm text-gray-700 whitespace-nowrap">{o.customer_name}</td>
              ...
              <div className="lg:hidden divide-y divide-gray-100">
                {orders.map((o) => (
                  <div key={o.id} className="p-4 hover:bg-gray-50/50 transition-colors">
                    <div className="flex items-start justify-between mb-2">
                      <div className="min-w-0 flex-1">
                        <div className="text-sm font-semibold text-gray-900">{o.order_no}</div>
                    ...
                    <div key={idx} className="p-3 bg-gray-50 rounded-lg">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm text-gray-600 break-words">{c.message}</p>
                        </div>
                      </div>
                    </div>
                  ))}
