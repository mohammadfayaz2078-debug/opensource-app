...
const ExpenseEdit = () => {
  ...
  const fetchExpense = async () => {
    ...
    try {
      ...
      Swal.fire(t('error'), t('expense.edit.load_failed'), 'error');
      navigate('/expenses');
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Swal.fire({
        icon: 'success',
        title: t('expense.edit.updated'),
        text: t('expense.edit.update_success'),
        timer: 1500,
      ...
      if (err.response?.status === 422) {
        ...
        Swal.fire(t('error'), t('expense.edit.form_errors'), 'error');
      } else {
        Swal.fire(t('error'), err.response?.data?.message || t('expense.edit.update_failed'), 'error');
      }
    } finally {
      setSaving(false);
    }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-gray-600">{t('expense.edit.loading')}</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500">{t('expense.edit.not_found')}</p>
        </div>
      </div>
    );
  }
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-semibold text-gray-800">{t('expense.edit.title')}</h1>
                <p className="text-xs text-gray-400 mt-0.5">
                  #{expense.id} • {expense.description || t('expense.edit.no_description')}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${
                ...
                {expense.account?.name || t('expense.edit.no_wallet')}
              </span>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">
                  ...
                  {t('expense.edit.expense_type')} <span className="text-red-500">*</span>
                </label>
                <select
                ...
                >
                  <option value="">{t('expense.edit.select_type')}</option>
                  {expenseTypes.map(t => (
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">
                  ...
                  {t('expense.edit.wallet')} <span className="text-red-500">*</span>
                </label>
                <select
                  name="account_id"
                ...
                >
                  <option value="">{t('expense.edit.select_wallet')}</option>
                  {accounts.map(a => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-3">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">
                  ...
                  {t('expense.edit.amount')} <span className="text-red-500">*</span>
                </label>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">
                  ...
                  {t('expense.edit.date')} <span className="text-red-500">*</span>
                </label>
                <input
                  type="date"
                  name="date"
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={t('expense.edit.vendor_placeholder')}
                />
              </div>
            </div>
            ...
            <div className="mt-3">
              ...
              <textarea
                ...
                placeholder={t('expense.edit.desc_placeholder')}
              />
            </div>
          </div>
          ...
          {expense && (
            <div className="bg-gray-50 rounded-xl border border-gray-100 p-4 mb-4">
              <h4 className="text-xs font-medium text-gray-600 mb-2">{t('expense.edit.info')}</h4>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                <div>
                  <span className="text-gray-400">{t('expense.edit.id')}</span>
                  <p className="font-medium text-gray-700">#{expense.id}</p>
                </div>
                <div>
                  <span className="text-gray-400">{t('expense.edit.created')}</span>
                  <p className="font-medium text-gray-700">
                    {new Date(expense.created_at).toLocaleDateString()}
                  </p>
                </div>
                <div>
                  <span className="text-gray-400">{t('expense.edit.type')}</span>
                  <p className="font-medium text-gray-700">
                    {expense.expense_type?.name || t('expense.edit.na')}
                  </p>
                </div>
                <div>
                  <span className="text-gray-400">{t('expense.edit.wallet')}</span>
                  <p className="font-medium text-gray-700">
                    {expense.account?.name || t('expense.edit.na')}
                  </p>
                </div>
              </div>
            </div>
            ...
            >
              {saving ? (
                <>
                  ...
                  {t('expense.edit.updating')}
                </>
              ) : (
                <>
                  <Save className="w-4 h-4 mr-2" />
                  {t('expense.edit.update')}
                </>
              )}
            </button>
          </div>
