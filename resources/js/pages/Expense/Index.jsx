...
const ExpenseIndex = () => {
  ...
  const handleDelete = async (expense) => {
    if (!confirm(t('expense.index.delete_confirm'))) return;
    try {
      await api.delete(`/expenses/${expense.id}`);
      fetchExpenses(currentPage);
    } catch (err) {
      alert(err.response?.data?.message || t('expense.index.delete_failed'));
    }
  };
    ...
    <div className="relative bg-gradient-to-br from-emerald-50/40 via-white to-sky-50/40 rounded-xl p-6 -m-6">
      ...
      <div className="mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <h1 className="text-xl font-semibold text-gray-900">{t('expense.index.title')}</h1>
            <p className="text-sm text-gray-500 mt-0.5">
              {t('expense.index.summary', { count: summaryData?.total_expenses || 0, amount: formatAmount(summaryData?.total_amount) })}
            </p>
          </div>
          <Link
            to="/expenses/create"
        ...
        <div className="flex flex-1 gap-2 flex-wrap">
          <div className="relative flex-1 min-w-[180px] max-w-xs">
            ...
            <input
              ...
              placeholder={t('expense.index.search_placeholder')}
          ...
          >
            <option value="">{t('expense.index.all_types')}</option>
            {expenseTypes.map(t => (
      ...
      {loading && (
        <div className="flex items-center justify-center py-12">
          ...
          <span className="ml-3 text-gray-700 text-sm">{t('expense.index.loading')}</span>
        </div>
      )}
          ...
          {expenses.length === 0 ? (
            <div className="py-16 text-center">
              ...
              <p className="text-sm text-gray-700">
                {searchQuery || typeFilter ? t('expense.index.no_match') : t('expense.index.no_results')}
              </p>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-100">
                      <th className="px-4 py-2.5 text-left text-xs font-medium text-gray-700 uppercase">{t('expense.index.date')}</th>
                      <th className="px-4 py-2.5 text-left text-xs font-medium text-gray-700 uppercase">{t('expense.index.type')}</th>
                      <th className="px-4 py-2.5 text-left text-xs font-medium text-gray-700 uppercase">{t('expense.index.description_vendor')}</th>
                      <th className="px-4 py-2.5 text-right text-xs font-medium text-gray-700 uppercase">{t('expense.index.amount')}</th>
                      <th className="px-4 py-2.5 text-left text-xs font-medium text-gray-700 uppercase">{t('expense.index.account')}</th>
                      <th className="px-4 py-2.5 text-right text-xs font-medium text-gray-700 uppercase">{t('expense.index.actions')}</th>
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      <tr key={expense.id} className="border-t border-gray-100 hover:bg-blue-50/50 transition-colors">
                        ...
                        <td className="px-4 py-2.5 whitespace-nowrap">
                          <span className="inline-flex items-center px-2 py-0.5 text-xs font-medium bg-blue-50 text-blue-700 rounded-full">
                            {expense.expense_type?.name || t('expense.index.uncategorized')}
                          </span>
                        </td>
                        <td className="px-4 py-2.5">
                          <div className="max-w-xs">
                            <p className="text-sm font-medium text-gray-800 truncate">
                              {expense.description || t('expense.index.no_description')}
                            </p>
                            {expense.paid_to && (
                              <p className="text-xs text-gray-400">{t('expense.index.vendor', { name: expense.paid_to })}</p>
                            )}
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-2.5 whitespace-nowrap text-right">
                          <div className="flex items-center justify-end gap-1">
                            <button onClick={() => navigate(`/expenses/${expense.id}/edit`)}
                              className="p-1.5 rounded hover:bg-yellow-50 text-gray-700 hover:text-yellow-600 transition-colors" title={t('expense.index.edit')}>
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                              </svg>
                            </button>
                            <button onClick={() => handleDelete(expense)}
                              className="p-1.5 rounded hover:bg-red-50 text-gray-700 hover:text-red-600 transition-colors" title={t('expense.index.delete')}>
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="lg:hidden divide-y divide-gray-100">
                {expenses.map((expense) => (
                  <div key={expense.id} className="p-4 hover:bg-gray-50/50 transition-colors">
                    ...
                    {expense.paid_to && (
                      <div className="text-xs text-gray-400 mb-2">{t('expense.index.vendor', { name: expense.paid_to })}</div>
                    )}
          ...
          {totalPages > 1 && (
            <div className="px-4 py-3 border-t border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div className="text-xs text-gray-500 text-center sm:text-left">
                {t('expense.index.page', { current: currentPage, total: totalPages })}
              </div>
              ...
              <div className="flex items-center justify-center sm:justify-end gap-2">
                <span className="text-xs text-gray-500">{t('expense.index.show')}</span>
                ...
                <span className="text-xs text-gray-500">{t('expense.index.per_page')}</span>
              </div>
            </div>
          )}
        </div>
