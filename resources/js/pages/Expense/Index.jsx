      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-5">
        {['draft', 'submitted', 'paid', 'cancelled'].map(status => (
          <div key={status} className="bg-white border border-gray-200 rounded-lg p-3">
            <p className="text-xs text-gray-500 uppercase tracking-wide capitalize">{status}</p>
            <p className="text-xl font-bold mt-1 text-gray-800">{summary[status] || 0}</p>
          </div>
        ))}
        <div className="bg-white border border-gray-200 rounded-lg p-3">
          <p className="text-xs text-gray-500 uppercase tracking-wide">Total Amount</p>
          <p className="text-xl font-bold mt-1 text-[#007c89]">{summary.total?.toFixed(2) || '0.00'}</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg shadow-sm mb-5">
        <div className="p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} className="px-3 py-2 text-sm border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-[#007c89]">
                  ...
                  expenses.map((expense) => {
                    ...
                    return (
                      <tr key={expense.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                        <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">{expense.reference_no}</td>
                        ...
                        <td className="px-4 py-3 whitespace-nowrap text-right">
                          <div className="flex items-center justify-end gap-1">
                            {(expense.status === 'draft' || expense.status === 'submitted') && (
                              <>
                                <button onClick={() => openEditModal(expense)} className="p-1.5 rounded hover:bg-yellow-50 text-gray-500 hover:text-yellow-600" title="Edit">
                                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                  </svg>
                                </button>
                                <button onClick={() => handleDelete(expense)} className="p-1.5 rounded hover:bg-red-50 text-gray-500 hover:text-red-600" title="Delete">
                                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                  </svg>
                                </button>
                                <button onClick={() => openPayModal(expense)} className="px-2 py-1 rounded bg-emerald-100 text-emerald-700 text-xs font-medium hover:bg-emerald-200" title="Pay">
                                  Pay
                                </button>
                                <button onClick={() => handleCancel(expense)} className="p-1.5 rounded hover:bg-orange-50 text-gray-500 hover:text-orange-600" title="Cancel">
                                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div key={expense.id} className="p-4 hover:bg-gray-50">
                      <div className="flex items-start justify-between gap-3 mb-2">
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-gray-500">{expense.expense_type?.name || '—'}</p>
                        </div>
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border ${status.color}`}>
                          {status.label}
                        </span>
            ...
            <form onSubmit={handleSubmit}>
              <div className="px-6 py-5 space-y-4">
                ...
                <div>
                  ...
                  <select name="expense_type_id" value={form.expense_type_id} onChange={handleInputChange}
                    className={`w-full px-3 py-2 text-sm border rounded-md focus:outline-none focus:ring-1 focus:ring-[#007c89] ${errors.expense_type_id ? 'border-red-500' : 'border-gray-300'}`} required>
                ...
                <div>
                  ...
                  <textarea name="description" value={form.description} onChange={handleInputChange} rows="2"
                    className={`w-full px-3 py-2 text-sm border rounded-md focus:outline-none focus:ring-1 focus:ring-[#007c89] ${errors.description ? 'border-red-500' : 'border-gray-300'}`} required />
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input type="date" name="date" value={form.date} onChange={handleInputChange}
                      className={`w-full px-3 py-2 text-sm border rounded-md focus:outline-none focus:ring-1 focus:ring-[#007c89] ${errors.date ? 'border-red-500' : 'border-gray-300'}`} required />
                  </div>
                </div>
                ...
                <div className="border-t border-gray-100 pt-4">
                  <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-3">Payment Details (for Create & Pay)</p>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                    ...
                    <div>
                      ...
                      <input type="text" name="payment_reference" value={form.payment_reference} onChange={handleInputChange}
                        className="w-full px-3 py-2 text-sm border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-[#007c89]" placeholder="Cheque #, ref..." />
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="px-6 py-4 border-t border-gray-200 flex flex-col sm:flex-row items-stretch sm:items-center justify-end gap-2 sm:gap-3">
                ...
                {!isEditing && (
                  ...
                  }} disabled={saving} className="px-4 py-2 text-sm bg-emerald-600 text-white rounded-md hover:bg-emerald-700 disabled:opacity-50 inline-flex items-center">
                    {saving ? (
