...
export default function SaleInvoice() {
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      <!DOCTYPE html><html><head><title>Invoice ${sale?.reference_no || ''}</title>
      <style>
      ...
      <div class="header">
        <div class="company-info"><h1>INVOICE</h1><p>Reference: ${sale?.reference_no || ''}</p></div>
        <div class="invoice-title"><h2>INVOICE</h2><p>Date: ${sale?.document_date || ''}</p>${sale?.due_date ? `<p>Due: ${sale.due_date}</p>` : ''}</div>
      </div>
      <div class="details">
      ...
      <div class="totals"><div class="totals-box">
        <div class="totals-row"><span>Subtotal:</span><span>${parseFloat(sale?.subtotal || 0).toFixed(2)}</span></div>
        <div class="totals-row"><span>Discount:</span><span>-${parseFloat(sale?.discount_value || 0).toFixed(2)}</span></div>
        <div class="totals-row"><span>Shipping:</span><span>${parseFloat(sale?.shipping_cost || 0).toFixed(2)}</span></div>
        <div class="totals-row total"><span>Total:</span><span>${parseFloat(sale?.total_amount || 0).toFixed(2)}</span></div>
        <div class="totals-row paid"><span>Paid:</span><span>${parseFloat(sale?.paid_amount || 0).toFixed(2)}</span></div>
        <div class="totals-row due"><span>Due:</span><span>${(parseFloat(sale?.total_amount || 0) - parseFloat(sale?.paid_amount || 0)).toFixed(2)}</span></div>
      </div></div>
      ${sale?.notes ? `<div class="footer"><p><strong>Notes:</strong> ${sale.notes}</p></div>` : ''}
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div ref={printRef} className="bg-white border border-gray-200 rounded-lg shadow-sm p-8">
        <div className="flex justify-between items-start mb-8 pb-6 border-b-2 border-[#007c89]">
          <div><h1 className="text-2xl font-bold text-[#007c89]">INVOICE</h1><p className="text-sm text-gray-500 mt-1">Reference: {sale.reference_no}</p></div>
          <div className="text-right"><h2 className="text-lg font-bold text-[#007c89]">INVOICE</h2><p className="text-sm text-gray-500">Date: {sale.document_date}</p>{sale.due_date && <p className="text-sm text-gray-500">Due: {sale.due_date}</p>}</div>
        </div>
        <div className="grid grid-cols-2 gap-6 mb-8">
          <div className="bg-gray-50 p-4 rounded-lg">
            <h3 className="text-xs font-medium text-gray-500 uppercase mb-2">Customer</h3>
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            <h3 className="text-xs font-medium text-gray-500 uppercase mb-2">Invoice Details</h3>
        ...
        <table className="w-full mb-8">
          <thead><tr className="bg-[#007c89] text-white"><th className="px-4 py-3 text-left text-xs font-medium uppercase">#</th><th className="px-4 py-3 text-left text-xs font-medium uppercase">Product</th><th className="px-4 py-3 text-right text-xs font-medium uppercase">Qty</th><th className="px-4 py-3 text-right text-xs font-medium uppercase">Delivered</th><th className="px-4 py-3 text-right text-xs font-medium uppercase">Unit Price</th><th className="px-4 py-3 text-right text-xs font-medium uppercase">Total</th></tr></thead>
        ...
        <div className="flex justify-end"><div className="w-72">
          <div className="flex justify-between py-2 text-sm"><span className="text-gray-600">Subtotal</span><span>{parseFloat(sale.subtotal).toFixed(2)}</span></div>
          <div className="flex justify-between py-2 text-sm"><span className="text-gray-600">Discount</span><span className="text-red-600">-{parseFloat(sale.discount_value).toFixed(2)}</span></div>
          <div className="flex justify-between py-2 text-sm"><span className="text-gray-600">Shipping</span><span>{parseFloat(sale.shipping_cost).toFixed(2)}</span></div>
          <div className="flex justify-between py-3 text-lg font-bold border-t-2 border-[#007c89] mt-2"><span>Total</span><span>{parseFloat(sale.total_amount).toFixed(2)}</span></div>
          <div className="flex justify-between py-2 text-sm text-green-600"><span>Paid</span><span>{parseFloat(sale.paid_amount).toFixed(2)}</span></div>
          <div className="flex justify-between py-2 text-sm text-red-600 font-bold"><span>Due</span><span>{unpaid.toFixed(2)}</span></div>
        </div></div>
        {sale.notes && <div className="mt-8 pt-4 border-t border-gray-200"><p className="text-sm text-gray-600"><strong>Notes:</strong> {sale.notes}</p></div>}
        <div className="mt-8 pt-4 border-t border-gray-200 text-center text-xs text-gray-400">Generated on {new Date().toLocaleDateString()}</div>
      </div>
    </div>
  );
}
